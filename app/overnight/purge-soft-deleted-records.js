...
const purgeSoftDeletedRecords = async (date = new Date()) => {
  try {
    ...
    const result = {
      ...
      toString () {
        return 'Purge deleted records.' +
          ` Success: ${this.count.success.dogs} Dogs ${this.count.success.owners} Owners - [${[...this.deleted.success.dogs, ...this.deleted.success.owners].join(', ')}].` +
          ` Failed: ${this.count.failed.dogs} Dogs ${this.count.failed.owners} Owners - [${[...this.deleted.failed.dogs, ...this.deleted.failed.owners].join(', ')}].`
      }
    }
    ...
    console.log('Error purging soft deleted records:', e)
    return {
      count: {
        success: new DeletedRecordCount(),
        failed: new DeletedRecordCount()
      ...
      toString () {
        return `Error purging soft deleted records: ${e}`
      }
    }
  }
}
