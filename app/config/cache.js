...
const schema = Joi.object({
  ...
  ttl: Joi.number().integer().default(DAY), // 24 hours
  tls: Joi.object(),
  segment: Joi.string().default(serviceName),
  expiresIn: Joi.number().default(DAY) // 24 hours
})
...
const config = {
  ...
  segment: process.env.REDIS_API_SEGMENT,
