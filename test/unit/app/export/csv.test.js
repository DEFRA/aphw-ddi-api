...
describe('CSV test', () => {
  ...
  test('convertToCsv should handle valid input row', async () => {
    ...
    expect(csvDataArray[13]).toBe('"John"')
    expect(csvDataArray[14]).toBe('"Smith"')
    expect(csvDataArray[16]).toBe('"new address line 1"')
    expect(csvDataArray[20]).toBe('"new postcode"')
    expect(csvDataArray[21]).toBe('"new country"')
    expect(csvDataArray[22]).toBe('"me@here.com"')
  })
...
const validHeader = '"IndexNumber","DogBreed","DogName","DogDateOfBirth","DogDateOfDeath","DogTattoo","DogColour","DogSex","DogMicrochip1","DogMicrochip2","DogExportedDate","DogStolenDate","DogUntraceableDate","OwnerFirstName","OwnerLastName","OwnerDateOfBirth","AddressLine1","AddressLine2","Town","County","Postcode","Country","Email","Telephone1","Telephone2","ExemptionStatus","CertificateIssued","CdoIssued","CdoExpiry","Court","PoliceForce","DogLegislationOfficer","ApplicationFeePaid","InsuranceCompany","InsuranceRenewalDate","NeuteringConfirmationDate","MicrochipVerificationDate","JoinedInterimSchemeDate","RemovedFromCdoProcess","ExemptionOrder","Withdrawn","ExaminedByDlo","MicrochipDeadline","NeuteringDeadline"'
