const {
  ...
  determineCreatePk, determineUpdatePk, sendDeleteToAudit, sendImportToAudit, sendChangeOwnerToAudit, sendPurgeToAudit
...
describe('SendAudit test', () => {
  ...
  describe('sendPurgeToAudit', () => {
    const hal9000 = { username: 'hal-9000', displayname: 'Hal 9000' }
    test('should fail given no user', async () => {
      await expect(sendPurgeToAudit('OBJECT', {}, {})).rejects.toThrow('Username and displayname are required for auditing deletion of OBJECT')
    })

    test('should send correct message payload', async () => {
      await sendPurgeToAudit('person', { personReference: 'P-123', id: 1, firstName: 'Test' }, hal9000)
      expect(sendEvent).toHaveBeenCalledWith({
        type: 'uk.gov.defra.ddi.event.purge',
        source: 'aphw-ddi-portal',
        partitionKey: 'P-123',
        id: expect.any(String),
        subject: 'DDI Purge person',
        data: {
          message: '{"actioningUser":{"username":"hal-9000","displayname":"Hal 9000"},"operation":"purged person","purged":{"personReference":"P-123"}}'
        }
      })
    })
  })
