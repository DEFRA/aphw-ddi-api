...
describe('Statistics repo', () => {
  ...
  describe('getCountsPerStatus', () => {
    test('should return counts per status', async () => {
      sequelize.models.dog.findAll.mockResolvedValue(mockCountsPerStatus)

      const counts = await getCountsPerStatus()

      expect(counts).toHaveLength(7)
      expect(counts[0].total).toBe('20')
      expect(counts[0].status.status).toBe('Interim exempt')
      expect(counts[1].total).toBe('30')
      expect(counts[1].status.status).toBe('Pre-exempt')
      expect(counts[2].total).toBe('40')
      expect(counts[2].status.status).toBe('Failed')
      expect(counts[3].total).toBe('500')
      expect(counts[3].status.status).toBe('Exempt')
      expect(counts[4].total).toBe('60')
      expect(counts[4].status.status).toBe('In breach')
      expect(counts[5].total).toBe('70')
      expect(counts[5].status.status).toBe('Withdrawn')
      expect(counts[6].total).toBe('80')
      expect(counts[6].status.status).toBe('Inactive')
    })
  })
